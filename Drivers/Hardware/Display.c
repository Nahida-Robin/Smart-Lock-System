...
/**
  * @brief 显示RTC设置模式（年/月/日/时/分/秒/星期）
  * @param NULL
  * @retval NULL
  */
...
/**
  * @brief 显示主菜单（RTC设置、成功/警告次数、传感器数据布局）
  * @param NULL
  * @retval NULL
  */
...
/**
  * @brief 显示温湿度数据
  * @param pData 传感器数据结构体指针
  * @retval NULL
  */
void Display_ShowDHT(const SensorData_t *pData)
{
	OLED_ShowNum(3, 1, pData->Temph, 2);
	OLED_ShowNum(3, 5, pData->Humih, 2);
}
...
/**
  ...
  * @retval NULL
  */
void Display_ShowSmog(const SensorData_t *pData)
{
	OLED_ShowNum(4, 1, (uint16_t)(pData->Smog), 4);
}
...
/**
  ...
  * @retval NULL
  */
void Display_ShowLight(const SensorData_t *pData)
{
	OLED_ShowNum(4, 9, pData->Light, 2);
}
